...
export function AppInput({ style, id: providedId, ...props }: InputHTMLAttributes<HTMLInputElement>) {
  ...
  return <input className="app-input" id={id} style={{ width: '100%', minHeight: 40, padding: '9px 13px', fontSize: 14, color: color.ink, background: color.surface, border: `1px solid ${props['aria-invalid'] ? color.danger : color.controlBorder}`, borderRadius: radius.md, ...style }} {...props} />;
}
